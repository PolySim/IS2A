...
print(cake |> group_by(recipe) |> summarise(moyenne_angle = mean(angle), ecart_type = sd(angle)))
cat("\nMoyennes par temperature\n")
print(cake |> group_by(temp) |> summarise(moyenne_angle = mean(angle), ecart_type = sd(angle)))
...
m_ri <- lmer(angle ~ recipe + temp_c + (1 | replicate), data = cake, REML = TRUE)
m_ri_ml <- lmer(angle ~ recipe + temp_c + (1 | replicate), data = cake, REML = FALSE)
m_rs <- lmer(angle ~ recipe + temp_c + (1 + temp_c | replicate), data = cake, REML = TRUE)
m_rs_ml <- lmer(angle ~ recipe + temp_c + (1 + temp_c | replicate), data = cake, REML = FALSE)
...
cat(
  ...
  "et la pente de temperature est positive (environ 0.158 deg d'angle par deg F). ",
...
cat("Prediction populationnelle (sans effet aleatoire) :", round(pred_moyenne, 3), "\n")
cat("Prediction conditionnelle pour le replicat 1 :", round(pred_rep1, 3), "\n\n")

cat("7) Intercept aleatoire utile ? Test du rapport de vraisemblance\n")
cat("---------------------------------------------------------------\n")
m_gls <- gls(angle ~ recipe + temp_c, data = cake, method = "ML")
m_lme <- lme(angle ~ recipe + temp_c, random = ~1 | replicate, data = cake, method = "ML")
print(anova(m_gls, m_lme))
cat(
  "\nConclusion : l'ajout d'un intercept aleatoire ameliore tres fortement le modele ",
  "(p-value < 0.0001).\n\n",
  sep = ""
)
...
acf_par_rep <- split(data.frame(temp = cake$temp, resid = res_rs), cake$replicate)
lag1 <- sapply(acf_par_rep, function(df) cor(df$resid[-1], df$resid[-nrow(df)]))
cat("\nResume des correlations lag-1 par replicat\n")
print(summary(lag1))

print(
  ggplot(data.frame(fitted = fit_rs, resid = res_rs), aes(x = fitted, y = resid)) +
    geom_point(alpha = 0.7) +
    geom_hline(yintercept = 0, linetype = 2) +
    labs(title = "Residus vs valeurs ajustees", x = "Valeurs ajustees", y = "Residus") +
    theme_minimal()
)
...
cat(
  "\nConclusion : les residus paraissent acceptables. La normalite n'est pas rejetee ",
...
boot_var <- bootMer(m_rs, var_fun, nsim = 200, use.u = FALSE, type = "parametric", seed = 123)
...
print(ci_var)
cat(
  "\nConclusion : la variance d'intercept est clairement non nulle, alors que la ",
  "variance de pente reste tres proche de zero. L'heterogeneite entre replicats ",
  "porte surtout sur le niveau moyen, pas sur la reponse a la temperature.\n\n",
  sep = ""
)

cat("11) Evolution des predictions selon la temperature et recette la plus resistante\n")
cat("---------------------------------------------------------------------------------\n")
...
print(emm_rec)
cat(
  "\nLes droites sont paralleles car il n'y a pas d'interaction recette:temperature. ",
  "La recette A presente les predictions les plus elevees en moyenne, donc elle ",
...
cat(
  ...
  "aleatoire inutile. Dans ce modele, les effets de recipeB et recipeC sont negatifs ",
...
dev.off()
cat("\nLes graphiques ont ete enregistres dans tp4_graphiques.pdf\n")
