...
dates = create_range_date("1949-01", "1961-01")
...
if __name__ == "__main__":
    data = np.log(load_data())
    a, b = estime_params(data)
    # display_tendance_lineaire(a, b)
    data_without_trend = remove_tendance_lineaire(data, a, b)
    # display_basic_data(data_without_trend)
    mean_residu = calc_mean_residu(data_without_trend)
    # print(f"Mean residu: {mean_residu}")
    # auto_correlation(data_without_trend)
    serie_diff = remove_with_dif(data)
    # display_diff(serie_diff)
    # display_tendance_mean_mobile(data)
