    ...
    def wait_if_needed(self):
        ...
        with self.lock:
            ...
            if len(self.request_times) >= self.max_requests:
                ...
                if wait_time > 0:
                    logger.warning(
                        f"Rate limit atteint, attente de {wait_time:.1f} secondes..."
                    )
            ...
            if self.total_requests % 50 == 0:
                logger.info(
                    f"Rate limiter: {len(self.request_times)} requêtes dans la dernière minute (total: {self.total_requests})"
                )
...
class MultiUserCalendarSync:
    def __init__(self, db_path="users_sync.db"):
        ...
        self.api_url_template = os.getenv(
            "API_URL", "https://votre-api.com/events?user={user_id}"
        )
        self.api_headers = {
            ...
            try:
                ...
                if e.resp.status == 429:  # Too Many Requests
                    ...
                    if attempt < max_retries:
                        delay = base_delay * (2**attempt)
                        logger.warning(
                            f"Erreur serveur {e.resp.status}, retry dans {delay}s (tentative {attempt + 1}/{max_retries + 1})"
                        )
                        time.sleep(delay)
                        continue
                    else:
    ...
    def add_user(
        self, email: str, user_api_id: str, calendar_id: str = "primary"
    ) -> bool:
    ...
    def fetch_api_events_for_user(self, user_api_id: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Requête API pour {user_api_id}: {url}")
            ...
            logger.info(
                f"Récupéré {len(events)} événements pour l'utilisateur {user_api_id}"
            )
            return events

        except requests.RequestException as e:
            logger.error(
                f"Erreur lors de la récupération des événements pour {user_api_id}: {e}"
            )
            return []

    def normalize_event(
        self, api_event: Dict, user_id: int
    ...
    def get_user_sync_state(self, user_id: int) -> Dict[str, Dict]:
        ...
        state = {}
        for row in cursor.fetchall():
            state[row[0]] = {"google_id": row[1], "summary": row[2]}

        conn.close()
        return state

    def update_sync_state(
        self, user_id: int, event_hash: str, google_event_id: str, summary: str
    ):
    ...
    def sync_user_events(self, user: Dict) -> Dict[str, int]:
        ...
        try:
            ...
            logger.info(
                f"Étape 1: Nettoyage des anciens événements pour {user['email']}..."
            )
                ...
                if old_hash not in api_events_dict:
                    ...
                    if "google_id" in old_data and old_data["google_id"]:
                        if self.delete_event(
                            service, user["calendar_id"], old_data["google_id"]
                        ):
                            stats["deleted"] += 1
                            self.remove_sync_state(user["id"], old_hash)
                            logger.info(
                                f"Supprimé: {old_data.get('summary', 'Événement sans titre')}"
                            )
                        else:
                            stats["errors"] += 1
                else:
                    ...
                    if api_summary != old_summary:
                        ...
                        if "google_id" in old_data and old_data["google_id"]:
                            if self.delete_event(
                                ...
                                self.remove_sync_state(user["id"], old_hash)
                                logger.info(
                                    f"Modifié (suppression): {old_summary} -> {api_summary}"
                                )
                            else:
                                stats["errors"] += 1
                    else:
                        ...
                        api_events_dict[old_hash]["google_id"] = old_data.get(
                            "google_id"
                        )

            # ÉTAPE 2: Ajouter les nouveaux événements
            logger.info(
                f"Étape 2: Ajout des nouveaux événements pour {user['email']}..."
            )
            for event_hash, event_data in api_events_dict.items():
                ...
                created_event = self.create_event(
                    service, user["calendar_id"], event_data["event"]
                )
                if created_event:
    ...
    def sync_all_users(self) -> bool:
        ...
        for user in users:
            logger.info(
                f"Synchronisation de {user['email']} (API ID: {user['user_api_id']})"
            )
        ...
        logger.info(
            f"Rate limiter - Requêtes dans la dernière minute: {rate_stats['requests_last_minute']}/{self.rate_limiter.max_requests}, "
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python multi_user_sync.py add_user <email> <user_api_id> [calendar_id]"
        )
    ...
    if command == "add_user":
        if len(sys.argv) < 4:
            print(
                "Usage: python multi_user_sync.py add_user <email> <user_api_id> [calendar_id]"
            )
            sys.exit(1)
        ...
        if users:
            ...
            for user in users:
                print(
                    f"  - {user['email']} (API ID: {user['user_api_id']}, "
